  ...
  */
  ...
  function _getRequestAuthContext_(e, data) {
    ...
    try {
      if (/^app1\./.test(String(token || '')) && typeof verifyAppSessionToken === 'function') {
        var appRes = verifyAppSessionToken(token);
        if (!appRes || !appRes.success) return { success: false, error: (appRes && appRes.error) ? appRes.error : 'App session verification failed', details: appRes };
        REQUEST_AUTH_CTX = {
          success: true,
          email: String(appRes.email || '').toLowerCase().trim(),
          emailVerified: !!appRes.emailVerified,
          tokenInfo: appRes.tokenInfo || { type: 'app_session' }
        };
   ...
   */
  function _isAdminSafe(userEmail) {
    try {
    ...
    try {
      ...
      var roles = String(user.roles || user.role || user.Role || '').toLowerCase();
      return roles.indexOf('admin') !== -1;
    } catch (err) {
      try { appLog('ERROR', '_isAdminSafe', String(err && err.message ? err.message : err)); } catch (ee) {}
      return false;
    }
  }

  function _isHMOrAdminSafe(userEmail) {
    try {
      if (typeof isHMOrAdmin === 'function') {
        if (isHMOrAdmin(userEmail)) return true;
      }
    } catch (e) {
      // ignore and fall back
    }
    try {
      if (_isAdminSafe(userEmail)) return true;
      var effectiveEmail = (REQUEST_AUTH_CTX && REQUEST_AUTH_CTX.email) ? REQUEST_AUTH_CTX.email : userEmail;
      var email = String(effectiveEmail || '').toLowerCase().trim();
      if (!email) return false;
      var list = _getCachedSheetData('Users').data;
      ...
      try { appLog('ERROR', '_isHMOrAdminSafe', String(err && err.message ? err.message : err)); } catch (ee) {}
      return false;
    }
  }
  ...
  */
  function doPost(e) {
    ...
    try {
      _clearRequestCache();
      return routePost(action, data, e);
    } catch (err) {
  ...
  */
  ...
  function _buildLessonPlanSubmissionEmailHtml(context, plans) {
    ...
    return [
      ...
      '<a href="https://workflow-wine.vercel.app/" style="display:inline-block;padding:12px 24px;background-color:#2563eb;color:white;text-decoration:none;border-radius:6px;font-weight:600;">📋 Review & Approve Lesson Plan</a>',
      '</p>',
      '<p style="color:#666;font-size:12px;">Navigate to Lesson Plans → Approve Lesson Plans tab</p>',
  ...
  */
  function _handleSubmitScheme(data) {
    try {
      ...
      // DUPLICATE CHAPTER CHECK: Prevent duplicate chapter names for same class/subject
      try {
        const allSchemes = _rows(sh).map(row => _indexByHeader(row, headers));
        const norm = (v) => String(v || '').toLowerCase().trim();
        const duplicateScheme = allSchemes.find(s => {
          return norm(s.class) === norm(data.class) &&
                 norm(s.subject) === norm(data.subject) &&
                 norm(s.chapter) === norm(data.chapter) &&
                 norm(s.teacherEmail) === norm(teacherEmail);
        });
        
        if (duplicateScheme) {
          const statusInfo = duplicateScheme.status ? ` (Status: ${duplicateScheme.status})` : '';
          return _respond({ 
            ok: false, 
            blocked: true,
            reason: 'duplicate_chapter',
            message: `Chapter "${data.chapter}" already exists for ${data.class} ${data.subject}${statusInfo}. Please use a different chapter name.`,
            error: `Duplicate chapter name not allowed`
          });
        }
      } catch (dupErr) {
        console.error('Duplicate chapter check error:', dupErr);
      }

      // GATE: block new scheme if previous scheme for same class/subject is incomplete
      if (!data.forceSubmit && !data.skipPreviousSchemeCheck) {
        try {
          const gate = canSubmitNewScheme(teacherEmail, data.class, data.subject);
          if (!gate.allowed) {
            return _respond({ ok: false, blocked: true, reason: gate.reason, message: gate.message, error: gate.message });
          }
        } catch (gateErr) {
          // fail-open: unexpected errors must not permanently block teachers
          console.error('canSubmitNewScheme gate error:', gateErr);
        }
      }
  ...
  function _handleUpdateSchemeStatus(data) {
    try {
      ...
      try {
        ...
        let actorRole = 'HM';
        try {
          if (actorEmail && isSuperAdmin(actorEmail)) actorRole = 'Admin';
        } catch (e) {}
  ...
  */
  /**
   * Check if submitted lesson-plan sessions for a chapter are contiguous.
   * Do not use Schemes.noOfSessions as a hard approval gate: teachers can submit
   * a smaller lesson-plan batch for review, and HM should be able to approve it.
   * Returns {complete: true} if submitted sessions have no gaps.
   */
  function _checkChapterSessionsComplete(schemeId, chapter) {
    try {
      ...
      if (chapterPlans.length === 0) {
        return { complete: false, missing: [], total: 0, submitted: 0, error: 'No lesson plans submitted for this chapter' };
      }
      ...
      if (existingSessions.size === 0) {
        return { complete: false, missing: [], total: 0, submitted: 0, error: 'No numbered sessions found for this chapter' };
      }

      const sessionNumbers = Array.from(existingSessions).sort((a, b) => a - b);
      const maxSubmittedSession = sessionNumbers[sessionNumbers.length - 1];
      
      // Find missing sessions only inside the submitted range. Do not block
      // future sessions that have not been submitted as part of this batch.
      const missingSessions = [];
      for (let i = 1; i <= maxSubmittedSession; i++) {
  ...
  function _handleUpdateLessonPlanStatus(data) {
    try {
      ...
      // ROLE CHECK: Only HM or Admin can approve lesson plans (status='Ready')
      if (status === 'Ready') {
        if (!requesterEmail) {
          return _respond({ error: 'Authorization required: User email must be provided for approval' });
        }
        
        const isAuthorized = _isHMOrAdminSafe(requesterEmail);
        if (!isAuthorized) {
          return _respond({ error: 'Unauthorized: Only Headmaster or Admin can approve lesson plans' });
        }
      }
      ...
      try {
        ...
        try {
          if (actorEmail && _isHMOrAdminSafe(actorEmail)) actorRole = 'HM';
        } catch (e) {}
  ...
  function _handleBatchUpdateLessonPlanStatus(data) {
    try {
      const ids = data.lessonPlanIds || data.lpIds || data.ids || [];
      const lessonPlanIds = Array.isArray(ids) ? ids.map(id => String(id || '').trim()).filter(Boolean) : [];
      const targetStatus = String(data.status || '').trim();
      const comments = String(data.reviewComments || data.remarks || '').trim();
      const requesterEmail = String(data.requesterEmail || data.email || data.approverEmail || '').trim().toLowerCase();

      if (!lessonPlanIds.length) {
        return _respond({ success: false, error: 'No lesson plans selected' });
      }
      if (!targetStatus) {
        return _respond({ success: false, error: 'Target status is required' });
      }
      ...
      if (!requesterEmail) {
        return _respond({ success: false, error: 'Authorization required: User email must be provided' });
      }
      if (!_isHMOrAdminSafe(requesterEmail)) {
        return _respond({ success: false, error: 'Unauthorized: Only Headmaster or Admin can update lesson plan approvals' });
      }
      ...
      const headers = _headers(sh);
      const rows = _rows(sh);
      const plans = rows.map(row => _indexByHeader(row, headers));
      const idSet = new Set(lessonPlanIds.map(id => id.toLowerCase()));
      const targets = [];

      for (let i = 0; i < plans.length; i++) {
        const p = plans[i];
        const rowId = String(p.lpId || p.lessonPlanId || p.id || '').trim();
        if (rowId && idSet.has(rowId.toLowerCase())) {
          targets.push({ idx: i, plan: p, lpId: rowId });
        }
      }

      const foundIds = new Set(targets.map(t => t.lpId.toLowerCase()));
      const notFound = lessonPlanIds.filter(id => !foundIds.has(String(id || '').toLowerCase()));
      if (targets.length === 0) {
        return _respond({ success: false, error: 'Selected lesson plans were not found', notFound: notFound });
      }
      ...
      if (targetStatus === 'Ready') {
        ...
        const blocked = [];
        targets.forEach(t => {
          const schemeId = String(t.plan.schemeId || '').trim();
          const chapter = String(t.plan.chapter || '').trim();
          if (!schemeId || !chapter) return;
          const key = schemeId + '|' + chapter.toLowerCase();
          if (checkedChapters[key]) return;
          const completeness = _checkChapterSessionsComplete(schemeId, chapter);
          checkedChapters[key] = true;
        ...
        if (blocked.length > 0) {
          ...
          return _respond({
            success: false,
            error: `Cannot approve: All ${first.total || ''} sessions must be submitted before approval. Missing sessions: ${(first.missing || []).join(', ')}`,
            incomplete: true,
            blocked: blocked
          });
        }
      ...
      let statusIdx = normH.findIndex(h => h === 'status');
      if (statusIdx === -1) statusIdx = normH.findIndex(h => h.toLowerCase() === 'status');
      if (statusIdx === -1) return _respond({ success: false, error: 'Status column not found in sheet' });
      ...
      targets.forEach(t => {
        const rowIndex = t.idx + 2;
        try {
          sh.getRange(rowIndex, statusIdx + 1).setValue(targetStatus);
          if (commentsIdx >= 0 && comments) sh.getRange(rowIndex, commentsIdx + 1).setValue(comments);
          if (reviewedAtIdx >= 0) sh.getRange(rowIndex, reviewedAtIdx + 1).setValue(nowISO);
          updated += 1;
          updatedIds.push(t.lpId);
          const te = String(t.plan.teacherEmail || '').toLowerCase().trim();
          if (te) teacherEmails[te] = true;
        } catch (writeErr) {
          failed.push({ lpId: t.lpId, error: String(writeErr && writeErr.message ? writeErr.message : writeErr) });
        }
      });

      try {
        Object.keys(teacherEmails).forEach(te => invalidateCache('teacher_lessonplans_' + te));
        invalidateCache('approved_schemes');
      } catch (invErr) {
      }
      ...
      try {
        logAudit({
          action: targetStatus === 'Ready' ? AUDIT_ACTIONS.APPROVE : (targetStatus === 'Rejected' || targetStatus === 'Needs Rework' ? AUDIT_ACTIONS.REJECT : AUDIT_ACTIONS.UPDATE),
          entityType: AUDIT_ENTITIES.LESSON_PLAN,
          entityId: 'batch:' + updatedIds.join(','),
          userEmail: requesterEmail,
          userName: requesterEmail,
          userRole: 'HM',
          description: `Batch lesson plan status changed to ${targetStatus} (${updated} updated)`,
          severity: AUDIT_SEVERITY.WARNING
        });
      } catch (auditErr) { /* ignore audit failures */ }
      ...
      return _respond({
        ...
        updated: updated,
        skipped: lessonPlanIds.length - updated,
        totalRequested: lessonPlanIds.length,
        lpIdsUpdated: updatedIds,
        notFound: notFound,
        failed: failed,
        message: updated + ' lesson plan(s) updated successfully'
      });
    } catch (error) {
      return _respond({ success: false, error: error.message });
    }
  ...
  function _handleRescheduleLessonPlan(data) {
    try {
      ...
      if (requesterEmail && !_isHMOrAdminSafe(requesterEmail)) {
    ...
    try {
      ...
      try {
        logAudit({
          ...
          userRole: _isAdminSafe(email) ? 'Admin' : 'HM',
  ...
  */
  ...
  /**
  * Handle batch sync of session dependencies (HM/Admin only)
  */
  ...
  */
  ...
  function _handleGetMissingSubmissionsTeacherwiseRange(params) {
    try {
      ...
      const requesterEmail = String(params.email || params.requesterEmail || '').toLowerCase().trim();
      if (!_isHMOrAdminSafe(requesterEmail)) {
        return _respond({ success: false, error: 'Permission denied. HM, Admin, or Admin access required.' });
      }
  ...
  */
  ...
  /**
   * Chapter-scoped bulk update of lesson plan statuses (HM/Admin only)
  ...
  function _handleChapterBulkUpdateLessonPlanStatus(data) {
    ...
    try {
      ...
      // ROLE CHECK: Only HM or Admin can perform bulk approval/rework/rejection.
      if (!requesterEmail) {
        return _respond({ success: false, error: 'Authorization required: User email must be provided' });
      }
 ...
 * These functions allow Admins to manage all system data
 */
      ...
      if (row.lessonPlanId === lessonPlanId) {
        ...
        try {
          logAudit({
            ...
            userRole: 'Admin',
            description: `Lesson plan deleted by Admin (${rowObj.class || ''} ${rowObj.subject || ''})`,
            severity: AUDIT_SEVERITY.CRITICAL
          });
        } catch (auditErr) { /* ignore audit failures */ }
      ...
      if (row.schemeId === schemeId) {
        ...
        try {
          logAudit({
            ...
            userName: String(requesterName || requesterEmail || '').trim(),
            userRole: 'Admin',
            description: `Scheme deleted by Admin (${rowObj.class || ''} ${rowObj.subject || ''})`,
            severity: AUDIT_SEVERITY.CRITICAL
          });
        } catch (auditErr) { /* ignore audit failures */ }
      ...
      if (row.reportId === reportId) {
        ...
        try {
          logAudit({
            ...
            userName: String(requesterName || requesterEmail || '').trim(),
            userRole: 'Admin',
            description: `Daily report deleted by Admin (${rowObj.class || ''} ${rowObj.subject || ''} Period ${rowObj.period || ''})`,
            severity: AUDIT_SEVERITY.CRITICAL
          });
        } catch (auditErr) { /* ignore audit failures */ }
...
function getAllUsers() {
  try {
    ...
    appLog('INFO', 'getAllUsers', 'Found ' + rows.length + ' user rows');
    
    const users = rows.map(r => _indexByHeader(r, headers));
    ...
    return { success: true, users: cleanUsers };
  } catch (err) {
