...
function doPost(e) {
  ...
  return _respond({ submitted: true, examId });
    ...
    // IT Lab - Generate draft substitutions for HM review
    if (action === 'generateITLabDrafts') {
      const date = (data.date || '').trim() || _isoDateString(new Date());
      return _respond(generateITLabDrafts(date));
    }
    
    // IT Lab - Get pending drafts for HM review
    ...
    // IT Lab - HM submits approved drafts
    ...
    // IT Lab - HM updates a draft (change teacher)
    if (action === 'updateITLabDraft') {
      const draftId = (data.draftId || '').trim();
      const newTeacher = (data.newTeacher || '').trim();
...
/**
 * Generates draft IT Lab substitutions for HM review
 * Saves to "IT Lab Drafts" sheet, not directly to Substitutions
 */
...
function updateITLabDraft(draftId, newTeacher) {
  try {
    const draftsSheet = _getSheet('IT Lab Drafts');
    const rowIndex = Number(draftId) + 2;
    const draftHeaders = _headers(draftsSheet);
