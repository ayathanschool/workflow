...
const ReportCard = ({ user }) => {
  ...
  useEffect(() => {
    ...
    try {
      // Get unique classes from loaded exams
      if (exams.length > 0) {
        const uniqueClasses = [...new Set(exams.map(exam => exam.class).filter(Boolean))];
    ...
    <div className="space-y-6">
      ...
      {user && hasAccess && (
        <div className="bg-white p-4 rounded-lg shadow-sm border space-y-4">
          ...
          {/* Class Selection (for HM in both class and student mode) - SECOND for HM */}
          {isHM && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Select Class
          ...
          {/* Exam Selection - THIRD (after class is selected for HM) */}
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Select Exam
            </label>
            <select
              ...
              disabled={examsLoading || (isHM && !selectedClass)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              <option value="">
                 {examsLoading ? 'Loading exams...' : 
                 (isHM && !selectedClass) ? 'Select class first...' :
                 exams.length === 0 ? 'No exams available' : 
                 'Choose exam...'}
              </option>
              {/* Show unique exam types */}
              {(() => {
                let filteredExams = exams;
                
                // Filter by class for HM and Class Teachers
                if (isHM && selectedClass) {
                  filteredExams = exams.filter(exam => exam.class === selectedClass);
                } else if (isClassTeacher) {
                  const userClass = user.classes?.[0] || user.classTeacherFor || '';
                  if (userClass) {
                    filteredExams = exams.filter(exam => exam.class === userClass);
                  }
                }
                
                // Get unique exam types
                const uniqueExamTypes = [...new Set(filteredExams.map(exam => exam.examType || exam.examName.split(' - ')[0]).filter(Boolean))];
                
                return uniqueExamTypes.map(type => (
          ...
          {(isClassTeacher || isHM)  && viewMode === 'student' && (
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
